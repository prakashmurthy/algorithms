class QuickSort
  ...
  def pick_pivot_index(array)
    ...
    return 0 if array_length <= 2
    middle_index = array_length/2 
    middle_index = middle_index -1 if array_length.even?
    middle_element = array[middle_index]

    return middle_index if ((middle_element < first_element) && (middle_element > last_element)) ||
                           ((middle_element < last_element) && (middle_element > first_element))

    return 0 if ((first_element < middle_element) && (first_element > last_element)) ||
                           ((first_element < last_element) && (first_element > middle_element))

    return array_length -1 if ((last_element < first_element) && (last_element > middle_element)) ||
                           ((last_element < middle_element) && (last_element > first_element))
  end
  ...
  def partition(input_array = @input_array)
    ...
    # Pick the pivot
    pivot_index = pick_pivot_index(input_array)
